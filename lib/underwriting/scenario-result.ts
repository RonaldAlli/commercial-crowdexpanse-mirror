...
export type DerivedScenario = { scenarioVersion: string; metrics: AnalysisMetrics };
...
export function deriveScenarioResult(assumptions: ResolvedAssumption[], lineage: ModelLineage): DerivedScenario {
  ...
  const metrics = computeAnalysis(assumptionsToAnalysisInputs(assumptions));
  return { scenarioVersion, metrics };
}
