// Focused E2E for Better Lists — sellers slice.
...
// same seller where/orderBy/skip/take the /sellers page builds. Proves: param
...
// Mirror of the /sellers page query, built from the real parsed params.
async function querySellers(orgId, raw) {
  const params = parseListParams(raw, { sortKeys: SORT_KEYS, defaultSort: "newest" });
  const where = { organizationId: orgId };
  if (params.hasQuery) {
    where.OR = [
      { name: ilike(params.q) }, { company: ilike(params.q) },
      { email: ilike(params.q) }, { city: ilike(params.q) },
    ];
  }
  const [total, rows] = await Promise.all([
    prisma.seller.count({ where }),
    prisma.seller.findMany({ where, orderBy: SORT_ORDER[params.sort], skip: params.skip, take: params.take }),
  ]);
  return { params, total, rows };
}

const mkSeller = (orgId, data) => prisma.seller.create({ data: { organizationId: orgId, ...data } });
...
try {
  ...
  await mkSeller(a.id, { name: "Marigold Trust" });                       // name
  await mkSeller(a.id, { name: "Nguyen Family", company: "Marigold Holdings" }); // company
  await mkSeller(a.id, { name: "Contact One", email: "owner@marigold.test" });   // email
  await mkSeller(a.id, { name: "Contact Two", city: "Marigold City" });          // city
  await mkSeller(a.id, { name: "Unrelated Seller" });
  const rSearch = await querySellers(a.id, { q: "marigold" });
  assert(rSearch.total === 4, "search matches name/company/email/city (4 hits)");
  assert((await querySellers(a.id, { q: "MARIGOLD" })).total === 4, "search is case-insensitive");
  assert((await querySellers(a.id, { q: "nonesuch" })).total === 0, "non-matching term → 0");

  console.log("\n[3] Org scoping:");
  await mkSeller(b.id, { name: "Marigold Rival" }); // same token, other org
  const rScoped = await querySellers(a.id, { q: "marigold" });
  assert(rScoped.total === 4, "org B's matching seller is not counted for org A");
  assert(rScoped.rows.every((s) => s.organizationId === a.id), "every row belongs to org A");

  console.log("\n[4] Pagination:");
  for (let i = 0; i < 25; i++) await mkSeller(a.id, { name: `Pageblock Seller ${String(i).padStart(2, "0")}` });
  const pg1 = await querySellers(a.id, { q: "pageblock", page: "1" });
  const pg2 = await querySellers(a.id, { q: "pageblock", page: "2" });
  ...
  await mkSeller(a.id, { name: "Alpha sortcheck Co" });   // created first → older
  await mkSeller(a.id, { name: "Zeta sortcheck Co" });    // created second → newer
  const byNewest = await querySellers(a.id, { q: "sortcheck", sort: "newest" });
  const byName = await querySellers(a.id, { q: "sortcheck", sort: "name" });
