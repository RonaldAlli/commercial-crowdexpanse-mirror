...
console.log(`\nD25 Deployment Engine — ${dryRun ? "DRY RUN (no live change)" : "DEPLOY"}`);
...
const result = await runDeploy(ctx, ops, { dryRun });
...
if (jsonOut) {
  ...
  console.log("\n── result ──");
  if (result.ok) console.log(dryRun ? "  ✅ DRY RUN OK — build + targets validated, live server unchanged" : `  ✅ DEPLOYED — serving ${result.buildId}`);
  else {
