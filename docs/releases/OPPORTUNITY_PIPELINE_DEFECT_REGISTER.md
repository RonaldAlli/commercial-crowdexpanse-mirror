...
**Status (2026-07-19):** Contract RATIFIED; OPP-1 + OPP-4 merged to `main`. **Slice 1 IMPLEMENTED
(server-side) — PENDING FOUNDER REVIEW** on branch `stabilize/opportunity-pipeline-slice1`. Per your
architecture direction, a dedicated **Stage Policy Evaluation layer** now sits between the UI and
persistence: `lib/stage-policy.ts` (pure rule engine → `ALLOW`/`REQUIRES_ATTESTATION`/`DENY` with
missing-truth/artifacts/explanation) + `lib/stage-policy-service.ts` (`evaluateStageTransition` +
