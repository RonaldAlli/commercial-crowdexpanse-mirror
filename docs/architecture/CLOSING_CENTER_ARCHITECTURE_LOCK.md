...
# Reserved concept — TX-0: Transaction Timeline (RATIFIED 2026-07-16 → see Slice 6 section below)

> **Status: RATIFIED 2026-07-16 — now Slice 6 (in implementation).** This reservation is superseded by the authoritative **Slice 6 — Transaction Timeline** section at the end of this document (decisions TX-0/TX-4/TX-5, invariants TL-1…TL-11). The paragraphs below are the original reservation, kept for provenance.
...
> **Delivery status:** **✅ LIVE in production 2026-07-16** (`main` @ `108a8bc`, **code-only — no migration, prod stays at 26 migrations**, serving `I1QNF8TL8U1e-EuGPHtp1`). Decision record: [Transaction Dashboard Decision Package](./TRANSACTION_DASHBOARD_DECISION_PACKAGE.md) (ratified). A **read-only, cross-opportunity, current-state PROJECTION** of every deal in-flight past `UNDER_CONTRACT` — their readiness, blockers, per-domain status, next/overdue milestone, and responsible party — with each row linking out to that Opportunity's Closing Center. It is **not** a new operational domain, a transaction record, a timeline, a workflow engine, or a materialized/cached reporting model. Verified: authenticated `/closing` renders (200) with filters + empty-state; prod schema drift empty; no writes; behavior re-proven on `_test` (`e2e-transaction-dashboard.mjs`, 21 assertions) + 46 Playwright visual tests.
...
**Decisions TD-A…TD-L** and **invariants TD-1…TD-12** are ratified as written in the [decision package](./TRANSACTION_DASHBOARD_DECISION_PACKAGE.md), with **TD-L resolved: roadmap #7 (Opportunity-list closing badges) is a SEPARATE follow-up slice** — it must reuse the same pure projection module but has its own implementation plan, UI review, Playwright coverage, verification gate, and release; **the Opportunity list is NOT modified in Slice 5.**

**Pre-release invariants (founder refinement, ratified 2026-07-16):**
- **TD-10 — Stable row ordering.** Rows are ordered by a **total, DB-order-independent** rule — never the database return order: (1) overdue first, (2) soonest next milestone (a dated row before an undated one, then date ascending), (3) pipeline stage order, (4) opportunity title, (5) opportunity id. This guarantees deterministic screenshots, Playwright, pagination, and UX. Implemented as the pure `sortTransactionRows` / `compareTransactionRows`.
- **TD-11 — Projection independence (graceful degradation).** A missing domain record (Escrow, Financing, Assignment, or the Closing Checklist) **never suppresses the Opportunity row** — the value degrades to `null` and the row still renders. No single domain's absence removes another's data.
- **TD-12 — Projection immutability.** The projection module accepts **plain data only** (never Prisma models, never mutable domain objects) and never mutates its inputs (`sortTransactionRows` returns a new array). This keeps the module a clean, reusable contract for roadmap #7.
...
# Slice 6 — Transaction Timeline (TX-0)

> **Status: ✅ LIVE in production 2026-07-16** (`main` @ `e955e8a`, **code-only — no migration, no schema change, no ActivityLog index; prod stays at 26 migrations**, serving `AkKD_n2EeTREsRoFafA-N`). Decision record: [Transaction Timeline Decision Package](./TRANSACTION_TIMELINE_DECISION_PACKAGE.md) (ratified). Verified: authed `/opportunities` + `/closing` + `/activity` render (200); prod schema drift empty; migration count unchanged; behavior proven on `_test` (16 unit cases + `e2e-transaction-timeline.mjs` 31 assertions + 7 Playwright tests). A **read-only, single-opportunity, chronological PROJECTION** of a deal's recorded history — stage moves, escrow, financing, assignment, checklist, document/offer-memo generation, and the underwriting decision — rendered on the Opportunity detail page as the **historical complement** to the current-state Closing Center. It is **not** a new domain, table, event bus, materialized view, second source of truth, workflow engine, or mutation surface. Where the **Dashboard (Slice 5)** is breadth across deals (current state), the **Timeline** is depth within one deal (history). Companion to `docs/architecture/TRANSACTION_DASHBOARD_DECISION_PACKAGE.md`; extends the Closing read model; touches neither the frozen V1.3 underwriting engine nor Slices 1–5's records.

**Ratified cornerstones (founder-articulated):**
- **TX-0** — the Timeline is a read-only, single-opportunity chronological projection of **already-recorded** events; **no new persistence, no writes, no new domain**.
- **TX-4 — Projection Composition (standing principle).** Every read surface over Closing state consumes the **same pure projection modules + domain helpers**; none rebuilds its own derivation. Dependency graph: *Operational Domains → Pure Projection Modules → { Dashboard, Timeline, Opportunity Badges (#7), Reporting }*. No duplicated readiness / blocker / milestone / status logic anywhere. The Timeline composes `lib/transaction-dashboard.ts` (stage order) and the `escrow`/`financing`/`assignment`/`closing` label·tone helpers; a shared pure stage/label helper may be extracted only *iff* a clean, behavior-neutral seam is warranted (reviewed on its own).
- **TX-5 — Projection Version (reserved only, NOT implemented).** A future identifier for a read surface's **rendering / ordering / aggregation semantics**, independent of the operational data it reads (the V1.3-lineage philosophy applied to read surfaces). Reserved so it can be introduced coherently across all TX-4 surfaces when a semantics-versioning need arises. **No runtime behavior in TX-0.**

**Persistence decision — Option A (RATIFIED): code-only, NO index this slice.** All timeline data already exists in `ActivityLog` (direct `opportunityId` + `createdAt` + `actor.name` + stored human-readable `eventLabel`/`eventBody`), written by every Closing domain's `audit()` helper (`escrow.*`, `financing.*`, `assignment.*`, `closing.*`, `opportunity.stage_changed`, `underwriting.decided`, `offer_memo.generated`, `assignment_agreement.generated`, `document.*`). TX-0 ships with **no migration, no new table, no event replication, no new writes**. The `@@index([organizationId, opportunityId, createdAt])` is **NOT** added in this slice — after release, benchmark realistic production usage and introduce it later as a dedicated additive optimization **only if** the benchmark demonstrates need ("prove first, optimize second").

**Locked invariants (TL-1…TL-11):**
- **TL-1** Source = `ActivityLog` filtered by `{ organizationId, opportunityId }` (direct key, no join for scoping).
- **TL-2** As-recorded boundary — the audit path is best-effort (`.catch(() => {})`), so the Timeline is presented as **recorded activity, never an authoritative or complete ledger**; authoritative state stays with the domains.
- **TL-3** Deterministic total ordering: `occurredAt` → stable category order → `eventLabel` → `id` (plain `<`/`>`, not locale) — stable screenshots/pagination even for same-millisecond writes.
- **TL-4** Pure module: plain data in, new arrays out, no clock, no Prisma, never mutates input.
- **TL-5** Reuse domain label/tone/stage/readiness helpers (TX-4); no parallel derivation.
- **TL-6** Read-only UI: no actions/mutation; entries may deep-link **OUT** only.
- **TL-7** Org-scoped + fail-closed opportunity-read authorization; **no new RBAC resource**.
- **TL-8** Immutable snapshots (`EscrowEvent` / `UnderwritingDecision` / generated docs) are optional future enrichment via reference, not a v1 data dependency.
- **TL-9** No migration by default (Option A); any index is additive, benchmark-gated (TX-A), behavior-neutral.
- **TL-10 — Event Integrity.** Every rendered entry corresponds to **exactly one existing persisted event**. The Timeline may group/organize events but must **never synthesize** an entry with no persisted record (e.g. a derived "closing nearly complete" is not a timeline entry unless it is itself a persisted event — aggregate status is the Dashboard's job).
- **TL-11 — Snapshot Reference.** When an entry corresponds to an immutable snapshot (`EscrowEvent`, `UnderwritingDecision`, a generated Assignment Agreement or Offer Memo), the entry **links back to the authoritative artifact** rather than copying its data. Projection, not duplication.
- **TL-12 — Unknown Event Forward Compatibility.** An `eventType` the timeline does not explicitly classify (e.g. a family added by a future Closing slice) is **displayed**, classified as **`other`**, with its recorded label/body **preserved** — it is **never silently discarded**. New event types surface without a code change.
- **TL-13 — Snapshot Link Failure.** If a referenced artifact is deleted or unavailable, the timeline **renders the entry and suppresses only the hyperlink** — it **never suppresses the event**. History survives missing references. Implemented as an I/O-free injected availability predicate (`isReferenceAvailable`) so the pure module stays clock-/I/O-free; omitted → references are treated as available (today's references target durable page anchors/routes, so none dangle — the seam future-proofs artifact-specific links).

**Ratified specifics:** **mount** = a read-only Timeline panel on the Opportunity detail page (`app/(workspace)/opportunities/[id]/page.tsx`), beside the Closing Center — **no separate top-level route**. **Default ordering** = newest-first, with an oldest-first toggle (a pure sort flip).
...
> **Status: ✅ LIVE in production 2026-07-16** (`main` @ `edd4824`, **code-only — no migration, no schema change, no dependency change; prod stays at 26 migrations**, serving `pgfa3y2AzXBEWuUkYQ6Fo`). Decision record: [Opportunity-List Closing Badges Decision Package](./OPPORTUNITY_LIST_BADGES_DECISION_PACKAGE.md) (ratified). Verified: authed `/opportunities` + `?view=list` render (200); prod schema drift empty; migration count unchanged; Board/Dashboard/Timeline untouched; behavior proven on `_test` (12 unit cases + `e2e-opportunity-badges.mjs` 16 assertions + 9 Playwright tests). The **final Closing read surface**: compact **Closing badges** on each Opportunity **List** row — readiness (✓ Ready / ⚠ N blockers / Closing not started) + per-domain Escrow / Financing / Assignment status — summarizing closing health at a glance and linking into the Closing Center. A pure **read projection** derived at read time from records each deal already owns; **not** a new domain, table, migration, index, service, cached readiness, or any recomputation of closing state. Completes the read model: Closing Center (one deal's workspace) · Dashboard (breadth across deals) · Timeline (one deal's history) · **List badges (health across the pipeline list)**.
