...
9. **Generalize only after a second real consumer exists.** Do not build a "generic" abstraction for one caller — a premature abstraction locks in guesses about a second use case that hasn't arrived. Build the first consumer concretely; when a genuine second consumer appears, generalize then. *Example (Slice 2, Commit 2b):* the per-field provenance component was Owner-specific until Property became a real second consumer, at which point it was renamed `owner-provenance.tsx` → `field-provenance.tsx` / `FieldProvenanceCard` — a behavior-preserving rename with two proven consumers, not a speculative one. The mirror-image failure is a "generic" helper with a single caller: that is coupling wearing an abstraction's clothes.
...
- **Intelligence UI is entity-parameterized reuse, not per-entity reimplementation.** A new entity's provenance display + manual-refresh surface consumes the *same* infrastructure the first entity does — `getFieldProvenance`, `listRefreshJobsForEntity(org, entityType, id)`, the shared `FieldProvenanceCard`, and an entity-specific refresh action that mirrors the first (swap `targetEntityType` + the entity's `SourceAdapter` + its projected field set). No second provenance system, no forked ledger. *(Slice 2 Commit 2b: Property detail reuses the Owner detail pattern wholesale.)*
- **Manual refresh: invalid input FAILS, it is not silently swallowed.** A refresh value that fails the adapter's normalization produces a *rejected* observation, which fails the whole run (`RefreshJob.status = FAILED` with a reason) — never a silent NOOP. NOOP is reserved for a *valid* value that equals the current projection (value-grain idempotency). Client `min`/`max`/numeric hints reduce invalid submits, but normalization stays server-authoritative in the adapter. *(Confirmed against actual behavior in Slice 2 Commit 2b — do not "fix" the code to convert rejection into NOOP.)*
...
- **Shared-component regression** — when a component is generalized to serve a second entity, prove statelessness + entity scoping by rendering it (`renderToStaticMarkup`) for entity A, then B, then A again: assert A's markup is **byte-identical** across the two A renders (no leaked module state) and B's markup carries only B's label/value. *Proven: 2b Owner → Property → Owner render regression.*
- **Production smoke without customer data** — after a release that first exposes an entity's UI, run an interactive/render smoke. If production has **no** real records of that entity, do **not** create customer-visible production records: use a clearly-designated **internal test organization** and remove the record immediately, or run the smoke against the **`*_test`** environment. Persistent production test records are never acceptable. Always also verify real deployed routes at the HTTP layer (protected routes redirect, not 5xx) as a liveness check independent of data.
