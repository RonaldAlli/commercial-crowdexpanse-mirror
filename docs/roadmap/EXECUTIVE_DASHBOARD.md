...
| Commercial Intelligence (Owner/Property/Market/Portfolio) | 🟡 In progress | 1.2 | 58% (**Slice 1 LIVE in production** — Owner foundation/UI/linking/candidate review/refresh/merge. **Slice 2 Commits 2a + 2b LIVE**: Property on the *shared* `Observation→Signal→Projection` spine + the **2b Property provenance UI + refresh surface** (generalized `FieldProvenanceCard`), closing [D13](./TECHNICAL_DEBT.md). **Commits 2c-i + 2c-ii landed HEADLESS** — 2c-i: canonical Property identity as a *derived, deterministic, fingerprinted* layer on the ledger (anchors + normalizers + rebuildable `PropertyIdentity` index + immutable crosswalk); **2c-ii: the Resolution engine** — a pure deterministic classifier + guarded resolve-before-create, append-only `PropertyResolution` audit + reversal, `PropertyMatchDecision` candidate store; prod at **13 migrations**; [D14](./TECHNICAL_DEBT.md) (headless, heals at 2c-iii). [D5](./TECHNICAL_DEBT.md) resolved. [Slice 1 acceptance](../releases/V1_2_SLICE_1_ACCEPTANCE.md). Next: Slice 2 Commit 2c-iii — identity/resolution review UI + redeploy) |
...
| 1.2 | Commercial Intelligence | 🟡 In progress — architecture locked ([Volume 12](./COMMERCIAL_INTELLIGENCE_ARCHITECTURE.md)); **Slice 1 LIVE in production** (1a…1d-3b; serving `9555QJiLxh4O9PrlVp3UH`; D5 resolved; [acceptance](../releases/V1_2_SLICE_1_ACCEPTANCE.md)). **Slice 2: Commits 2a + 2b LIVE; 2c-i + 2c-ii landed headless** — Property on the shared spine + the **2b provenance UI/refresh (deployed)**, plus **2c-i Property identity foundation** (derived deterministic `PropertyIdentity` index + immutable crosswalk) and **2c-ii the Resolution engine** (pure deterministic classifier + guarded resolve-before-create + append-only audit/reversal + candidate store); prod **13 migrations**, [D14](./TECHNICAL_DEBT.md) headless. Next: Slice 2 Commit 2c-iii (review UI + redeploy). |
