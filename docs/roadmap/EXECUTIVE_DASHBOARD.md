...
| Deal Analyzer / Underwriting | 🟡 Partial | 1.3 | 45% (**Commit 3a LIVE** — the canonical deterministic ownership model `Underwriting → Scenario → Assumption → ScenarioResult` with model lineage + `scenarioVersion` fingerprint, one-way ScenarioSeed snapshot, rebuildable ScenarioResult, and a behavior-preserving analyzer repoint onto it; `lib/analysis.ts` kernel unchanged. Prod **14 migrations**. [Underwriting Architecture Lock](../architecture/UNDERWRITING_ARCHITECTURE_LOCK.md) (UW-1…UW-9). Math-deepening slices (NOI/cap/debt/cash-flow/sensitivity/risk/reports) still ahead.) |
...
| 1.3 | Commercial Underwriting | 🟢 In progress — architecture locked ([Underwriting Architecture Lock](../architecture/UNDERWRITING_ARCHITECTURE_LOCK.md), UW-1…UW-9). **Commit 3a LIVE** (Underwriting Model Formalization): canonical `Underwriting → Scenario → Assumption → ScenarioResult`, model lineage + deterministic `scenarioVersion`, one-way ScenarioSeed snapshot, rebuildable ScenarioResult, `UNDERWRITING` RBAC, behavior-preserving analyzer repoint; `lib/analysis.ts` unchanged; additive migration, prod **14 migrations**, serving `deN1uqROM2_cQSQ7XUiaj`. Deferred: deeper math (NOI/cap/debt/cash-flow/sensitivity/risk), findings/risks + decided recommendation (3d), reports, `DealAnalysis` table removal ([D15](./TECHNICAL_DEBT.md)). |
