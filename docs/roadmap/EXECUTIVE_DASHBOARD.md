...
| Deal Analyzer / Underwriting | 🟡 Partial | 1.3 | 52% (**3a + 3b-i LIVE** — 3a: canonical deterministic ownership model `Underwriting → Scenario → Assumption → ScenarioResult` (model lineage + `scenarioVersion` fingerprint, one-way ScenarioSeed snapshot, rebuildable ScenarioResult, behavior-preserving analyzer repoint; `lib/analysis.ts` unchanged). **3b-i: deterministic debt sizing** (pure sibling module — LTV/LTC/DSCR → binding constraint; lineage bumped to v2; DS-1/DS-2). Prod **15 migrations**. [Architecture Lock](../architecture/UNDERWRITING_ARCHITECTURE_LOCK.md) (UW-1…UW-9, DS-1/DS-2) + [Calculation Principles](../architecture/UNDERWRITING_CALCULATION_PRINCIPLES.md). Remaining 3b: income/expense schedules → cash flow → exit/waterfall → sensitivity → findings/risks.) |
...
| 1.3 | Commercial Underwriting | 🟢 In progress — architecture locked ([Underwriting Architecture Lock](../architecture/UNDERWRITING_ARCHITECTURE_LOCK.md), UW-1…UW-9, DS-1/DS-2) + [Calculation Principles](../architecture/UNDERWRITING_CALCULATION_PRINCIPLES.md). **3a LIVE** (Underwriting Model Formalization — canonical ownership model, `scenarioVersion`, ScenarioSeed snapshot, rebuildable ScenarioResult, `UNDERWRITING` RBAC; `lib/analysis.ts` unchanged). **3b-i LIVE** (deterministic debt sizing — pure sibling `debt-sizing.ts`, LTV/LTC/DSCR → binding constraint, lineage v2). Prod **15 migrations**, serving `6MVWWstmgUbkMrQIKKrDy`. Remaining 3b (sequenced): income/expense schedules (3b-ii) → cash flow (3b-iii) → exit/waterfall (3b-iv) → sensitivity (3b-v) → findings/risks + recommendation (3b-vi). Deferred: decided recommendation (3d), reports, `DealAnalysis` removal ([D15](./TECHNICAL_DEBT.md)). |
