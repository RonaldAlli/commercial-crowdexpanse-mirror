...
> **Convention (binding):** the harness is now a permanent platform, not a one-off. Every UI slice from v1.4 onward MUST use it — see the [Visual / Browser UI Testing standard](../standards/VISUAL_TESTING.md) for the required fixtures / auth-state / viewport / screenshot-naming / retention / cleanup conventions. Do not introduce a second browser-testing tool.
