...
- **Deterministic identity fingerprint:** the derived index carries `identityVersion` — a deterministic fingerprint of **`IDENTITY_ALGORITHM_VERSION` + the winning anchor set + the anchor normalizer versions** (never wall-clock/sequence). This makes *evidence*, *normalization*, and *algorithm* changes each independently detectable without diffing, while keeping the derived identity a pure function of the ledger. Full detail: [Property Identity Lock §5, R5/R6](../architecture/PROPERTY_IDENTITY_LOCK.md).
...
- **Projection writes are transactional.** `Observation → Signal → Projection` **commit atomically** — never a signal without its recomputed projection, nor a projection without its signal. A failure rolls back the whole chain.
- **Projection is deterministic.** Given an identical ledger, identical precedence rules, and identical versions, the projection output is **always identical**. The precedence ordering is total (a signal-id final tiebreak guarantees exactly one winner), so `recompute` and `rebuild` are pure functions of the ledger.
- **Projection purity — the ProjectionService never creates information.** It may only **select, order, normalize, and project** existing accepted Signals. No heuristics, no inference, no guesses, no AI — those belong to the later scoring (Slice 5) and AI (2.0) layers. This is what keeps projection deterministic forever.
- **Projection ≠ Presentation.** Projection produces **canonical operational values only**; it knows nothing about colors, badges, formatting, display labels, or UI grouping. The UI (1d) *consumes* projected values — it never influences them. This separation keeps future read APIs clean.
- **Refresh is observational.** A refresh may **create Observations, create Signals, supersede Signals, and trigger Projection** — nothing else. It **never creates, merges, or splits Owners, and never deletes anything**. Ingestion introduces *new facts*; it does not change *identity or structure*. This keeps the ingestion layer (1c+) narrowly scoped and safe to run repeatedly.
- **Refresh is replayable.** Given an identical adapter, identical payload, and identical versions, running a refresh twice produces the **same ledger and projection state** — beyond mere idempotency, deterministic replay. Guaranteed two ways: **job-grain** idempotency (a unique `(org, sourceKey, requestKey)` — client key or server content-hash — returns the prior job instead of re-applying it) and **value-grain** idempotency (a value equal to the current lineage head is skipped, recording neither observation nor signal). Running the same refresh twice leaves the ledger **byte-for-byte identical** after the first successful run.
- **Adapters are pure.** A `SourceAdapter` only **`fetch`es** (its sole I/O boundary) and **`map`s** (a pure transform to candidate observations). It **never writes the database, calls the ProjectionService, or performs permission checks** — all mutation belongs to the refresh orchestrator. Every adapter exposes `sourceKey`, `sourceCategory`, and **`adapterVersion`**, and `adapterVersion` is **recorded on every Observation** it produces, so any datum is traceable to the exact adapter logic that emitted it. This is what lets every future source (CSV, county, licensed, AI) plug in as *just another adapter* while the orchestrator, ledger, and projection engine stay fixed.
- **Same-lineage USER_ENTERED supersession (the override rule).** A pinned `USER_ENTERED` signal stays **sticky against lower-precedence automated sources** (PUBLIC/LICENSED/CALCULATION) — an override protects a human-corrected value from being silently replaced by a provider. But a **newer deliberate `USER_ENTERED` value may supersede the previous `USER_ENTERED` signal**, because it represents an *explicit human correction* of an earlier human value. A manual refresh is exactly such a correction (it is `USER_ENTERED`), so it supersedes the prior user value by the normal lineage rule — the orchestrator adds **no** special pin-protection logic (that would duplicate precedence policy in the coordination layer). Cleanly: *human correction → may replace a prior human value; automated update → cannot replace a pinned human value.*
- **The UI never writes projections directly.** Every edit flows `Owner action → Owner service (`lib/owners`) → Observation → Signal → ProjectionService → UI re-read`. The presentation layer never sets `displayName`/`entityType`/`matchKey` on the Owner row — it calls `createOwner`/`updateOwnerField`/`clearOwnerOverride` and reads back the projected result. The ledger stays the single source of truth; the UI is a *consumer* of the domain model, never a second implementation of it.
- **The UI is stateless.** The UI never caches intelligence decisions. Every page load reflects the **current** projection, provenance, and permissions — no client-side copy becomes authoritative. (Owner pages are `force-dynamic`; there is no client store of owner/signal state.) This is what guarantees that future enrichment — provider adapters, scoring, portfolio, AI — appears in the UI automatically through the services, with no presentation-layer cache to invalidate.
- **Candidate ≠ Merge.** A *candidate* is a **proposal** — "these **may** be the same owner" — surfaced from normalized-name matching, visible to read roles, and never mutating anything. A *merge* is an **ADMIN decision** — "these **are** the same owner" — structural, reversible, ADMIN-only. The UI keeps the two workflows visually and navigationally **distinct**: a candidate surface never offers a merge action, and a merge surface never masquerades as a suggestion. (Presentation mirrors architecture: on Owner detail, **Projected Value → Winning Signal → Signal History**, echoing Projection → Signal → Observation.)
- **The UI reflects the pipeline.** Every Owner field shown on screen remains explainable through the hierarchy **Projection → Winning Signal → Observation lineage** — *what* the current value is (projection), *why* it has that value (the winning signal: source category, asOf, confidence, override), and *where* it came from (the observation/source it traces to). The presentation is permanently organized this way, so the architecture stays legible to users, not just developers. *(Known future UI refinement, not an engine change: when a field has no alternate accepted signal to fall back to, the "Clear pin" control should be disabled/hidden with copy like "No alternate signal exists to fall back to." The projection engine is unchanged — it simply keeps projecting the only accepted value.)*
- **Candidate Review records human decisions only.** The candidate-review workflow (Commit 1d-2b) surfaces *possible* owner duplicates (exact `matchKey` + alias overlap only — no fuzzy matching) and records a human's judgement as an `OwnerMatchDecision` (`CONFIRMED`/`DISMISSED`, with ADMIN reopen). It is decision-support, not resolution. Concretely, Candidate Review **never mutates Owner identity, never creates or deletes Owners, never invokes `mergeOwners`, and never writes Observations or Signals** — it only writes `OwnerMatchDecision` rows. **Confirm records a decision; it does not merge.** A `CONFIRMED` decision is merely the *input* to the Merge workflow (1d-3). Dismissed pairs stay suppressed until a **material identity change** (a change to the pair's `matchKey`/normalized-alias fingerprint) or an **explicit ADMIN reopen**. **Structural identity change remains exclusive to the Merge workflow** — the only place `mergeOwners` runs.
- **Linking never changes identity.** Linking may attach a Seller to an Owner, attach a Property to an Owner, or remove those links — it only edits the mutable operational graph (`Seller.ownerId`/`Property.ownerId`). It **never creates, merges, or resolves identity**; canonical identity stays solely the Owner identity subsystem's responsibility (candidate → decision → merge). A **move** (re-link A→B) is a single atomic `ownerId` update — there is never an intermediate committed `null` state — and produces **no Observation and no Signal** (the ledger is untouched). This is the operational-graph counterpart to "merge is structural only."
...
- **Merge is the only workflow permitted to perform structural identity change.** Only the Merge workflow (Commit 1a-2 engine + 1d-3b ADMIN controls) may repoint the canonical graph (`mergedIntoId`, operational-link repointing) and tombstone an owner. Linking (operational FK only), Candidate Review (decisions only), and Refresh/Projection (values only) **never** change identity structure. Merge is **ADMIN-only** (`canMergeOwners`), reversible, and structural-only. Winner selection may be **suggested deterministically** (more linked records → older record → smaller id; provider-neutral — external identifiers never decide the survivor) but is **never applied automatically**: an ADMIN explicitly confirms or swaps the winner.
- **Merge and its decision-resolution are one atomic transaction.** A merge **consumes** the `CONFIRMED` `OwnerMatchDecision` that authorized it — stamping `resolvedAt`/`resolvedByUserId`/`mergeRecordId` (the status stays `CONFIRMED`; there is no `MERGED` decision status). The merge engine and this resolution write commit or roll back **together**; a merge can never persist while its decision still shows "awaiting merge." The reverse also holds: **unmerge and decision-unresolution are one atomic transaction** — unmerge clears exactly the resolution linked to that merge record (`mergeRecordId` is unique) and returns the still-`CONFIRMED` pair to the queue, so an unmerge can never persist while its decision still shows "resolved." The `OwnerMergeRecord` (+ the decision's resolution stamp) is the authoritative merge/unmerge audit; the UI never records a duplicate event.
...
**Slice 2 Resolution-layer decisions (Commit 2c-ii, locked 2026-07-15)** — full list in the [lock doc §3.3](../architecture/PROPERTY_IDENTITY_LOCK.md). Resolution is a **deterministic classification engine** between identity structure (2c-i) and human workflow (2c-iii), not itself a workflow:

| # | Decision | Resolution |
|---|---|---|
| **RES-1** | Classification is pure | The classifier is a pure function of `(evidence, lookup results)` — no DB, clock, randomness, writes, or side effects; identical inputs ⇒ identical outcome. |
| **RES-2** | Deterministic sequence | Every resolution follows **Normalize → Lookup → Conflict Inspection → Classification → Decision → Attachment → Audit → Candidate → Rebuild**; attachment never precedes classification. |
| **RES-3** | Resolution basis | Every resolution/candidate records a deterministic `basis` (`UNIQUE_PARCEL` · `UNIQUE_EXTERNAL_IDENTIFIER` · `PARCEL_CONFLICT` · `ADDRESS_PROPOSAL` · `EXTERNAL_ID_CONFLICT`) — explanatory metadata, never an input to behavior, never a score. |
| **RES-4** | Authority hierarchy | **parcel ↓ external identifier ↓ address.** An external id joins Tier 1A only as a single conflict-free authoritative match (an anchor, not an override); a weak address disagreement downgrades only when it introduces another *authoritative* candidate. |
| **RES-5** | Resolution never modifies evidence | Resolution only **appends** (crosswalk, enrichment observations with original source metadata, audit + candidate rows) and deterministically rebuilds the index — it determines **where new evidence attaches**, never rewriting historical evidence. |
| **RES-6** | **Resolution replay is deterministic** *(invariant)* | Given identical `requestKey` + evidence + lookup state, the engine yields the **identical** target Property, `ResolutionBasis`, audit record, and candidate state. **Replay correctness is stronger than duplicate suppression.** |
| **RES-7** | **Reversal never rewrites classification history** *(invariant)* | A reversal **appends** a `REVERSAL` event + crosswalk revocation (`ACTIVE → SUPERSEDED` via `revokedByResolutionId`) + reversal metadata; it never mutates or deletes the original `RESOLVE` event or its `basis`. The historical classification stays true — only its operational effect changes. |

Authorization (Commit 2c-ii): automatic guarded Tier-1A resolve stays under ordinary `PROPERTY` create (a deterministic, reversible mechanism, not a discretionary decision); human decision + reversal actions are `PROPERTY_IDENTITY` (ADMIN + ACQUISITIONS, explicit read tier for the review surface; ANALYST/DISPOSITIONS excluded).
