...
**Next — Commit 2c (Property Identity):** architecture **🔒 locked 2026-07-15** — **[Property Identity — Architecture Lock](../architecture/PROPERTY_IDENTITY_LOCK.md)** (hybrid ledger-backed anchors + derived rebuildable identity index + immutable crosswalk; guarded `deterministic resolve-before-create` on `(FIPS, APN)`; crosswalk-first, no structural merge in 2c). Headless-first: **2c-i** anchors/normalizers/derived index/crosswalk → **2c-ii** guarded resolve-before-create + candidate generation → **2c-iii** review/resolution UI. Entry checkpoint: [V1.2 Midpoint Architecture Status](../architecture/V1_2_ARCHITECTURE_STATUS.md). **Next artifact: the Commit 2c planning brief (planning only — no code/migrations).**
