...
- ⚠️ **Headless — write-path not yet live** ([D13](./TECHNICAL_DEBT.md)): the ledger write-path is in the merged code but the running app is **not yet redeployed** (2a is a headless data-layer landing), so property edits still use the old direct-column path and no Property provenance/refresh UI exists. The redeploy lands with **2b**.

**Next — Commit 2b:** the thin Property-detail provenance UI + manual-refresh surface (mirrors the Owner detail pattern), including the app redeploy that makes the 2a write-path live.
