...
**Commit 2c (Property Identity):** architecture **🔒 locked 2026-07-15** — **[Property Identity — Architecture Lock](../architecture/PROPERTY_IDENTITY_LOCK.md)** (hybrid ledger-backed anchors + derived rebuildable identity index + immutable crosswalk; guarded `deterministic resolve-before-create` on `(FIPS, APN)`; crosswalk-first, no structural merge in 2c). Headless-first split:
- ✅ **2c-i — anchors / normalizers / derived `PropertyIdentity` index / immutable crosswalk — COMPLETE · MERGED · PRODUCTION-MIGRATED (headless, prod 12 migrations):** the typed projected-field model (`integer | string-anchor`), deterministic versioned normalizers (APN/FIPS/address), raw-preserved provenance, the derived index (deterministic `identityVersion` fingerprint over `IDENTITY_ALGORITHM_VERSION` + winning anchors + normalizer versions; deterministic projection watermark; content-idempotent zero-write rebuild), and the append-only-with-supersession `PropertyExternalIdentifier`. `PROPERTY_IDENTITY` RBAC defined (no reachable enforcement yet). No resolution behavior, no UI ([D14](./TECHNICAL_DEBT.md)).
- **2c-ii** — guarded resolve-before-create + deterministic candidate generation (next).
- **2c-iii** — review/resolution UI (redeploy that makes 2c-i live, closes D14).

Entry checkpoint: [V1.2 Midpoint Architecture Status](../architecture/V1_2_ARCHITECTURE_STATUS.md). **Next artifact: the Commit 2c-ii architecture/planning step.**
