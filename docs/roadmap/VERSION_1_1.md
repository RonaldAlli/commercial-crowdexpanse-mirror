...
> **Status:** 🟡 In progress (~75%). Testing/CI and Better Lists largely done; permissions and performance remain.
...
### 2. Permissions — 🟡 partial
Role model exists (`ADMIN`/`ACQUISITIONS`/`ANALYST`/`DISPOSITIONS`) with last-admin protection.
- **Remaining:** a documented **permission matrix** (who can create/edit/delete each record type and move pipeline stages); enforce it in server actions and hide disallowed UI; complete Team Management Slice 2 (member lifecycle) and Invitations (email delivery, resend/expiry UX).
- See [Team Management](./MODULE_ROADMAPS.md#team-management) and [Invitations](./MODULE_ROADMAPS.md#invitations).
...
- [ ] Permission matrix documented and enforced in server actions.
