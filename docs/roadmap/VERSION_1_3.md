...
- **Commit 3b — Deepen the deterministic financial engine — 🟢 in progress.** Deepens the math only (never ownership/lifecycle/governance/V1.2), as a sequence of deterministic sub-slices, each a **pure sibling** to `lib/analysis.ts` held to the [Calculation Principles](../architecture/UNDERWRITING_CALCULATION_PRINCIPLES.md): **3b-i debt sizing** → **3b-ii income/expense schedules** → **3b-iii financing cases + cash flow** → **3b-iv exit valuation + basic equity waterfall** → **3b-v sensitivity** → **3b-vi findings/risks + suggested recommendation** (all shipped). Remaining 1.3: the decided recommendation + `UNDERWRITING_APPROVAL` (3d) and offer-memo/LOI export.
  ...
  - **3b-iv — Exit Valuation + Basic Equity Waterfall — ✅ LIVE (2026-07-15).** The next projection layer per case (`… → CashFlowYear → Exit Valuation → Equity Cash Flows → Return Metrics`): pure `lib/underwriting/exit.ts` computes terminal value (exit-year NOI ÷ exit cap rate), selling costs, **amortized remaining loan balance** (true amortization, not a shortcut), net sale proceeds, the equity cash-flow series (year 0 contribution → annual CFBT → final year = operating CF + sale, counted once), and returns (equity multiple, **levered IRR** via deterministic bisection, total profit). Basic single-holder waterfall only. New OPERATING exit assumptions (`EXIT_CAP_RATE_PCT`/`SELLING_COSTS_PCT`), +11 `FinancingCaseResult` fields + new `EquityCashFlowYear` table; lineage bumped **v4→v5**; exit inputs feed each case's fingerprint via `scenarioVersion`. `lib/analysis.ts` unchanged. Invariants **EX-1** (extends, never rewrites lower layers) + **EX-2** (terminal value independently reproducible) + **EX-3** (debt payoff from frozen loan terms + exit timing) + **EX-4** (returns are pure functions of the equity series; IRR/multiple never inputs) + **EX-5** (no double-counting the final year) + **EX-6** (exit belongs to exactly one FinancingCase). Excludes refinance, promotes, preferred, catch-ups, multiple partners, tax, sensitivity, findings, approval. Additive migration (prod 17→18, serving `5j9m7oFkLcSOnmJGrnVmM`).
  - **3b-v — Sensitivity Matrices — ✅ LIVE (2026-07-15).** A per-`FinancingCase` sensitivity layer that is a pure **CONSUMER** of the engine (Principles 10–11), not another calculator: `SensitivityAnalysis` (one immutable baseline case) → `SensitivityCell` grid. Pure `lib/underwriting/sensitivity.ts` generates deterministic evenly-spaced axis values (≤11/axis, ≤121 cells) and, via an `evaluate` closure, re-derives a chosen target metric (levered IRR / equity multiple / total profit / cap rate / DSCR) over each cell's overrides — applied to an **in-memory COPY** of the frozen assumptions, **never persisted** — storing only the resulting reading. Fixed axis + metric allow-lists (D-C); `sensitivityVersion = f(financingCaseVersion, canonical spec, lineage)`; lineage bumped **v5→v6**. The baseline cell is marked **only on exact axis match**, never snapped. `lib/analysis.ts` unchanged. Invariants **SE-1** (never mutates its baseline) + **SE-2** (one immutable baseline FinancingCase) + **SE-3** (each cell a pure function of frozen assumptions + explicit overrides + lineage) + **SE-4** (cells disposable/rebuildable, zero-write when unchanged) + **SE-5** (deterministic axis generation) + **SE-6** (exact-only baseline marking) + **SE-7** (evaluation, not optimization). Configurator UI + rendered matrix; excludes findings/optimization/Monte Carlo/>2 axes. Additive migration (prod 18→19, serving `zjI3uM305qGrQXyBe27h3`).
  - **3b-vi — Findings / Risks + Suggested Recommendation — ✅ LIVE (2026-07-15).** The **top** consumer layer of the stack — interpretation, not calculation. Pure `lib/underwriting/findings.ts` runs a **fixed, versioned ruleset** over the settled deterministic outputs (operating `ScenarioResult` + each `FinancingCase`'s financing/exit results) and emits per-`Scenario` `ScenarioFinding`s + one advisory `ScenarioRecommendation` (`PROCEED` / `PROCEED_WITH_CONDITIONS` / `PASS`). It never feeds back into a calculation (Principle 7 / UW-7) and the engine never decides (UW-4). Findings belong to the Scenario (may cite a `FinancingCase`); financing/return findings are *decisive* only for the primary case (position 0). **Ratified R-A fingerprint separation**: `RULESET_VERSION` was **removed** from `computeScenarioVersion` / `computeFinancingCaseVersion` / `computeSensitivityVersion` and moved into a dedicated `computeFindingsVersion(scenarioVersion, caseVersions, rulesetVersion)` — so a rules-only change re-fingerprints ONLY the findings and never invalidates a metric, cash flow, valuation, financing result, IRR/waterfall, or sensitivity output (**FR-6**). Lineage: model 6 / calc 6 / **RULESET 1→2** (a pure ruleset bump; the kernel + model shape are unchanged). New `FindingSeverity`/`FindingCategory`/`RecommendationLevel` enums + `ScenarioFinding` + `ScenarioRecommendation` models; `rebuildFindings` runs last in the layered pipeline, content-idempotent. `lib/analysis.ts` unchanged. Invariants **FR-1** (pure fn of settled metrics + versioned ruleset) + **FR-2** (belong to exactly one Scenario) + **FR-3** (deterministic/rebuildable/disposable/idempotent) + **FR-4** (rules fixed + versioned; a rule change is a `RULESET_VERSION` bump) + **FR-5** (engine reports/recommends, humans decide) + **FR-6** (a ruleset change never alters a deterministic result). Excludes market-signal/external risks, decided recommendation/approval, offer-memo, configurable rules, ML/narrative. Additive migration (prod 19→20, serving `_1sMu2d-5oU-wzGWiQSqc`).
