...
| D15 | **Deprecated `DealAnalysis` table retained (v1.3, Commit 3a)** | 3a introduced the canonical `Underwriting → Scenario → Assumption → ScenarioResult` model and repointed the analyzer; `DealAnalysis` is no longer read or written but the table + `Opportunity.analysis` relation are retained (0 prod rows) so the ownership change can be proven stable across an accepted release before removal (founder decision D-7). An idempotent `backfillUnderwritingFromDealAnalysis` migrates any rows. | Low — drop the `deal_analysis` table + relation in a post-acceptance cleanup migration once the new model is confirmed stable in production. |
