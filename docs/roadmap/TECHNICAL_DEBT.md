...
| D13 | **Property ledger write-path merged but not redeployed (headless Slice 2 Commit 2a)** | The new `lib/properties.ts` write-path routes `yearBuilt`/`squareFeet` through the ledger, but the **running app was not rebuilt/redeployed** at 2a (a deliberate headless data-layer landing). Until redeploy, property edits use the *old* direct-column path, so an edit in the 2a→2b window changes a column with **no matching signal** → reconstruction drift for that field. **Exposure is nil today** (prod has 0 properties). | Fix at **Commit 2b** — the redeploy makes the ledger write-path live; **re-run the idempotent genesis backfill after redeploy** to heal any gap edits. |
