...
| D14 | **Property identity code merged + prod-migrated but app not redeployed (headless Slice 2 Commits 2c-i + 2c-ii)** | 2c-i landed the identity data layer (anchor projections, derived `PropertyIdentity` index, immutable crosswalk; prod **11 → 12**); **2c-ii** then landed the resolution layer — guarded resolve-before-create, `PropertyResolution` audit log, `PropertyMatchDecision` candidate store, reversal (prod **12 → 13**). Both are **headless**: the **running app was not rebuilt/redeployed** (mirrors 2a/D13). The anchor-write path, per-property identity-row creation, and the resolution engine are therefore **not live** in the running build. `createProperty` is wired through `resolveOrCreateProperty` but **inert** in the old build (no anchor/external-id inputs → Tier NONE → ordinary create); no candidate-review, reversal, or resolution UI is exposed. A property created in the 2c-i→2c-iii window by the old build would lack a `PropertyIdentity` row until a rebuild. **Exposure is nil today** (prod has 0 properties). | Fix at **Commit 2c-iii** — the UI redeploy makes the identity write-path + resolution engine live; **re-run `rebuildAllPropertyIdentities` after redeploy** to heal any gap. |
