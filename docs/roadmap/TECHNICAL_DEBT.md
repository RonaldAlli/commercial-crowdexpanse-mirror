...
| D5 | **`.next` nested files root-owned on host → production frontend is stale** | Plain `npm run build` can't overwrite `.next` (root-owned), so the PM2 app (`next start` from `.next`, build dated 2026-07-11) still serves the **pre-1.2 frontend** — the **entire 1.2 Owner UI (Slice 1 complete: 1d-1 core UI, 1d-2a linking, 1d-2b candidate review, 1d-3a refresh controls, 1d-3b merge/unmerge controls)** is merged, built (`.next-isolated`), and prod-DB-migrated (**10 migrations**) but **not live / not user-accessible**. Backend/migrations deploy fine (additive, backward-compatible); only the served frontend lags. **This is now the sole blocker to Slice 1 being usable in production.** | **Elevated (was low).** Deploy path: `sudo chown -R deploy:deploy .next` → `npm run build` → `pm2 restart crowdexpanse-commercial`; **or** repoint the PM2 app to `.next-isolated` (`NEXT_DIST_DIR=.next-isolated next start`) as an interim. Needs an operator with sudo — flagged for the maintainer, not auto-applied. |
