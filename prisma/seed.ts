...
const adminEmail = (process.env.ADMIN_EMAIL ?? "operator@commercial.crowdexpanse.com").trim().toLowerCase();
const adminPassword = process.env.ADMIN_PASSWORD;
const adminName = process.env.ADMIN_NAME ?? "Commercial Admin";

async function main() {
  if (!adminPassword) {
    throw new Error("Set ADMIN_PASSWORD before seeding the admin user.");
  }
